...
class Money():
    ...
    def inrange(self,num:int) -> int:
        return num in range(1,max(self.running)+1) 
...
class Game():
    ...
    def lose(self,bet: int) -> list[int]:
        ...
        print(f"{ColoredText(f"{player.name} suffered blood loss",'red')} {ColoredText(f"-{bet} drops",'light_red')}\n")
    ...
    def blackjack(self,xhand: Stack) -> True: 
        '''take argument of either player or dealer stack instance, generate a tuple of assignments based on provided argument,
        return true if the stack's stacksum attribute value is 21, while only 2 cards in list'''
        n,n2,xsum,ysum = self.tuple_reference_generator(xhand)
        if xsum == 21 and xhand.length_stack() == 2:
            return True 
    
    def bust(self,xhand: Stack) -> True:
        '''take argument of either player or dealer stack instance(*_hand), generate tuple assignments based on provided argument,
        return true if stack's stacksum attribute value is greater than 21 '''
        n,n2,xsum,ysum = self.tuple_reference_generator(xhand)
        if xsum > 21:
            return True
    
    def hit(self,xhand: Stack ) -> tuple[list,int]:
        ''' take either the provided player's or dealer's hand stack object instance (*_hand) and append/extend to its list a card drawn from the deck stack. return both new 
        list and its sum. return the new stack list and stacksum value as tuple 
        '''
        return xhand.extendstack(deck.rmcard_frmstack()), xhand.summ_cardss()

    def deal(self) -> tuple[list[str],list[int],list[str]]:
    ...
    def h1_gt_h2(self,xhand: Stack) -> True:
        '''takes 1 argument, the player or dealer's hand stack instance (*_hand), and returns true if the stack's stacksum attribute is greater than non-provided hand's stacksum attribute value, while still 
        less than or equal to 21.'''
        n,n2,xsum,ysum = self.tuple_reference_generator(xhand)
        if (ysum < xsum <= 21):
            return True

    def h1_eq_h2(self) -> True:
        '''evaluates if player_hand has same stacksum attribute as that of dealer_hand'''
        if player_hand.stacksum == dealer_hand.stacksum:
            return True 

    def h1_lt_h2(self,xhand: Stack) -> True:
    ...
    def can_hit(self,xhand: Stack) -> True:
        if not self.bust(xhand):
            return True
    ...
    def outcome(self,bet: int) -> None:   
        ...
        if self.h1_eq_h2():
            ...
            self.win(bet)
        elif self.bust(dealer_hand) or self.h1_gt_h2(player_hand):
            self.win(bet)
        elif self.bust(player_hand): 
    ...
    def tuple_reference_generator(self,xhand: Stack) -> tuple[str,str,int,int]:
    ...
    def must_hit(self,xhand: Stack) -> Stack: 
        ...
        while (xsum < ysum < 21):
            ...
            print(ColoredText(f"{n} says...hit me!...\n","light_cyan"))
            self.hit(xhand)
            time.sleep(2)
            self.outmsg()
            xsum = xhand.stacksum
...
def main():
    ...
    while game.game_play():
        ...
        game.deal()
        game.outmsg()
        blackj_boolst = game.boolist([game.blackjack(x) for x in (player_hand,dealer_hand)])
        if not any(blackj_boolst):
            game.must_hit(player_hand)
            if game.can_hit(player_hand):
                game.opt_hit()
                if not game.bust(player_hand):
                    game.standing()
